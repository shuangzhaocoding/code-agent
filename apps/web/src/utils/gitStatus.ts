...
export function gitMarkKind(code: string): GitMarkKind {
  if (code.includes('U') || /^(AA|DD|AU|UA|DU|UD)$/.test(code.trim())) return 'conflict'
...
export function gitMarkLabel(code: string): string {
  ...
  if (code.includes('U') || /^(AA|DD|AU|UA|DU|UD)$/.test(code.trim())) return '冲突'
...
export function gitMarkLetter(code: string): string | null {
  ...
  if (raw.includes('U') || /^(AA|DD)$/.test(raw)) return null
